...
def hypnogram_txt2new(input_file, output_file):
    ...
    for line in lines[1:]:  # 忽略文件的第一行（表头）
        ...
        if previous_stage != stage:
            annotations.append(f"{current_time:>7},{duration:>7},{stage_name.strip() :>14}")
        ...
        # 若睡眠阶段未改变则更新持续时间
            ...
            # 更新注释
            annotations[-1] = f"{parts[0]}, {float(parts[1]) + duration:>7}, {parts[2].strip() :>14}"
            # 更新当前时间
            current_time += duration
...
def convert_directory_AASM(directory):
    for filename in os.listdir(directory):
        if fnmatch.fnmatch(filename, '*.txt'):  # 确保处理.txt文件
            ...
            output_file = os.path.join(directory, filename[:-4] + '_RK_mne_Annotation.txt')
...
def main():
    ...
    directory = r"C:\Users\Tian_Yumi\Downloads\BaiduNetdiskDownload\sleep_data_PSG(depression)\2019.12.10-2019.12.11\2017-2019\汉字\depression\\"
    # 调用函数转换目录中的每个文件
