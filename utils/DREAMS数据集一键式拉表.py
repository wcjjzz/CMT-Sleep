...
def new_signal_extract(edf_path, hypnogram_path, channel='eeg1', filter=True, freq=[0.2, 40]):
    all_channels = ('ECG','FP1-A2','CZ-A1','EMG1','EOG1','VTH','VAB','NAF2P-A1','NAF1','PHONO','PR','SAO2','PCPAP','POS','EOG2','O1-A2','FP2-A1','O2-A1','CZ2-A1','EMG2','PULSE','VTOT','EMG3')

    first_sub_flag = 0

    data = [DREAMS_path+edf_path, DREAMS_path+hypnogram_path]
    print("preparing: " + data[0] + " " + data[1])
    signal2idx = {"eeg1": 1, "eeg2": 2, "eog": 4, "emg": 3}
...
DREAMS_path = 'D:\\AIGC\\DREAMS数据集\\DatabaseSubjects\\'
result_path = f"results/statistic_DREAMS.xlsx"
use_online_mne = False
edf_hyp_list=[]
...
for filename in os.listdir(DREAMS_path):
    if fnmatch.fnmatch(filename, 'subject*.edf'):
        tmp = filename.split('.')[0]
        for i in os.listdir(DREAMS_path):
            if fnmatch.fnmatch(i, 'mne_*AASM*'+tmp+'.txt'):
                edf_hyp_list.append((filename,i,))
                print(filename+" matched " +i)
for e,h in edf_hyp_list[0:]:
        # if [sub,day_]  in ignore_data:
        #     continue
        eeg1_1, labels_1, len_1, eeg1_m1, eeg1_std1 = new_signal_extract(e,h, channel='eeg1', filter=True,
                                                                     freq=[0.2, 40])
        eog_1, _, len_1, eog_m1, eog_std1 = new_signal_extract(e,h, channel='eog', filter=True, freq=[0.2, 40])
        num_seq = 15
        ...
        description = f"DREAMS:subject {e}"
