...
hospital_path = r"C:\Users\Tian_Yumi\Downloads\BaiduNetdiskDownload\sleep_data_PSG(depression)\2019.12.10-2019.12.11\2017-2019\汉字\depression\\"
...
#【改】======================================================================
    ...
    for filename in os.listdir(hospital_path):
        if fnmatch.fnmatch(filename, '*_RK_mne_Annotation.txt'):  # 查找以 "_RK_mne_Annotation.txt" 结尾的文件
            tmp = filename.split('_RK_mne_Annotation.txt')[0]  # 提取文件名前缀部分，比如 chenfang20170510
    ...
    # 遍历5个fold
    for i, (train_index, test_index) in enumerate(kf.split(edf_anno_list)):
        # 获取当前fold的信号文件和注释文件列表
        train_files = [edf_anno_list[i] for i in train_index]  # 信号文件和注释文件的元组列表


        # 调用 signal_extract_sequential_hospital 函数
        main_ext_raw_data, main_labels, main_sub_len, main_mean, main_std = signal_extract_sequential_hospital(
            train_files, channel='eeg1', filter=True, freq=[0.2, 40], stride=3)
